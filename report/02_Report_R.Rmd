...
val_data_clean <- data[-train_index, ]
```
...
To address potential multicollinearity issues, we extend our regression exploration to Ridge, LASSO, and Elastic Net.
We evaluate our model's performance using k-fold cross-validation on training data (2013–2020) followed by out of sample validation on data from 2021 to assess the generalisation of our model.
...
We train our model on previous data (2013-2020) and then use out-of-sample testing on 2021 to assess our model's fit.
*We use a quadratic term to capture curvature the curvature.*
...
## Test/train
Using the test/train split we outlined earlier:
