...
This will avoid any temporal leakage and isolate our true values for 2021, ensuring that the model does not have access to unseen future data, as we would expect in a real forecasting scenario.
However, our model may under perform for non-European or post-2021 data due to changes we may not have observed in this historical data.
...
After carrying out a literature review on how to improve our model, we came across a paper on using  @CHEN2019104934, 
...
To improve our model, we build upon the ideas of linear regression by regularising, this means we penalise larger coefficients in order to control the variance and reduce the effects of multicollinearity. 
