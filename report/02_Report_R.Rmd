...
clean_data_europe= select(-pm25_concentration) %>%
na.omit(europe_data)
```
...
clean_data_europe_reg$predicted_pm10 <- predict(modelcv, newdata = clean_data_europe_reg)
#now our results:
results=data.frame(
  actual_pm10 = clean_data_europe_reg$pm10_concentration,
  predicted_pm10 = clean_data_europe_reg$predicted_pm10,
  country = clean_data_europe_reg$country_name)
```
...
ridge_model <- train(lm_formula_pm10,data = clean_data_europe_reg,method = "glmnet",
  trControl = trainControl(method = "cv", number = 10),
  tuneGrid = expand.grid(alpha = 0,lambda = seq(0.0001, 1, length = 50)))

#now LASSO:
lasso_model <- train(lm_formula_pm10,data = clean_data_europe_reg,
  method = "glmnet",trControl = trainControl(method = "cv", number = 10),
  tuneGrid = expand.grid(alpha = 1,lambda = seq(0.0001, 1, length = 50)))

#and elastic net:
elastic_model <- train(lm_formula_pm10,data = clean_data_europe_reg,
method = "glmnet",trControl = trainControl(method = "cv", number = 10),
...
#we combine these to see:
best_params <- data.frame(
  Model = c("Ridge", "LASSO", "Elastic Net"),
  Alpha = c(0, 1, elastic_model$bestTune$alpha),
  Lambda = c(ridge_model$bestTune$lambda,
             lasso_model$bestTune$lambda,
             elastic_model$bestTune$lambda)
)
knitr::kable(best_params, caption = "Best alpha and lambda values for each method")
