...
This means we can effectively assess how well the model generalises to unseen future data, as we'd expect in a real world scenario, demonstrating its robustness. However, our model may under perform for non-European or post-2021 data due to changes we may not have observed in this historical data. 
...
We saw from our model that the cross-validated linear regression achieved an R^2 of 0.4845 on the training set. This means that the model explains 48.45% of the variance in pm10 concentrations across European cities.
...
Hence, we haven't fitted the optimal model since our assumptions have been violated. 
...
test_data_clean$predicted_pm10 <- predict(modelcv, newdata = test_data_clean)
#now our results:
results=data.frame(
...
To improve our model, we build upon the ideas of linear regression by regularising, this penalises large coefficients, providing adjustments for multicollinearity.
...
- Ridge: Reduces the size of all of the coefficients, but keeps them in the model. We have correlation between some predictors so this method should smooth out any instabilities this causes. ($\alpha=0$)

- Lasso: This method is similar to Ridge, but can shrink some coefficients all the way to zero, automatically removing less important variables. $\alpha=1$
...
We can see from our plot that the optimal regularisation parameter is 0.3061918, as beyond this point the RMSE CV increases with lambda (reg parameter), before that it behaves in the same way as our linear regression.
...
Similar to Ridge, we can see that the RMSE increases as the regularisation paramter increases, with our best results being around lambda=0.04091224, but again this doesn't provide very much difference from our original liner regression model. 
...
plot(elastic_model)
```
By combining via elastic net, we see we use approx 88.89% lasso, and approximately 11.11% Ridge, providing a regularisation parameter of lambda= 0.04091224. 
...
We see that the Elastic Net and LASSO methods have the lowest RMSE (Root Mean Squared Error) of 5.806026, meaning they produce the smallest average prediction error on the unseen data, making thm potentially more accurate.

Regarding the R^2 value, we can see again that Elastic and LASSO have the highest value, suggesting that when we tested our model on our unseen data, it explained 47.55% of the variation in pm10 concentrations across the european cities. 

Meanwhile the method with the smallest MAE (Mean Absolute Error) is the Ridge and Elastic method . Meaning we can deduce that overall these methods deviate the least from our true pm10 concentrations.

Hence we can conclude that the Elastic Net regularisation linear regression is the best model for our regression, since it strikes the optimal balance between our regularisation methods, with the lowest RMSE, highest R^2 and lowest MAE. 

So although the differences are very small, it strikes the best balance between bias and variance which provides slightly better predictive accuracy. 

## Conclusions

To conclude our section on linear regression, we found our initial linear regression model wasn't too accurate, only accounting for 41% of our data. To resolve this we regularised our linear regression model by using ridge, lasso and elastic net. Although these didn't provide too much improvement (due to the fact there was less multicolinearity between variables than we anticipated), theElastic Net method appeared to provide the best results to optimise our linear regression model. 

We also used transforms of variables, such as log population and polynomial terms for both the no2 concentration and year variables. This helped to capture the non-linearity aspects of our model. 
