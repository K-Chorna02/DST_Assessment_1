...
europe_pm10 <- europe_data %>%
  ...
  select(country_name, year, pm10_concentration, no2_concentration, population) 
...
- `no2_concentration` and `population` -predictor variables.
...
rf_model <- randomForest(
  formula = pm10_concentration ~ no2_concentration + population, 
  data = train_data
)
...
diff metric later) - **Mean of Squared Residuals = 103.87** -
representing the average squared difference between predicted and actual
PM10 values. - **% Variance Explained = 25.59%** - indicating that just
over one-quarter of the variance in PM10 levels is explained by NO2
