...
train_data_final_model <- filter(clean_data_europe, year <= 2020)
```
...
cat("R2 (RBF - Validation - Best):", r2_rbf_valid_best, "\n")
...
```{r}
# Final RBF SVR Model (best gamma & cost)
svr_rbf_model_final <- svm(
  pm10_concentration ~ year * no2_concentration + population + latitude + longitude,
  data = train_data_final_model,
  type = "eps-regression",
  kernel = "radial",
  gamma = 0.01,  # best gamma from tuning
  cost = 1
)
...
# Create UTF-8 copies for tooltips only
tooltip_city <- iconv(test_data$city, from = "", to = "UTF-8", sub = "?")
tooltip_country <- iconv(test_data$country_name, from = "", to = "UTF-8", sub = "?")
...
p_rbf_test <- ggplot(test_data, aes(
  ...
  labs(
    x = "Actual PM10",
    y = "Predicted PM10",
    title = "RBF SVR Predictions vs Actual (Test Data, >2020)"
  ) +
  theme_minimal()
...
```{r}
# Evaluation Metrics
actuals_rbf_test <- test_data$pm10_concentration

rmse_rbf_test <- sqrt(mean((predictions_rbf_test - actuals_rbf_test)^2))
mae_rbf_test <- mean(abs(predictions_rbf_test - actuals_rbf_test))
r2_rbf_test <- 1 - sum((predictions_rbf_test - actuals_rbf_test)^2) /
                    sum((actuals_rbf_test - mean(actuals_rbf_test))^2)

cat("RMSE (RBF - Test):", rmse_rbf_test, "\n")
cat("MAE (RBF - Test):", mae_rbf_test, "\n")
cat("R2 (RBF - Test):", r2_rbf_test, "\n")
```
